...
| 路径 | 内容 | 对应阶段 |
| --- | --- | --- |
| `01-sgemm-tutorial/src/kernels/naive_sgemm.cuh` | 三重循环 baseline | 阶段 8 |
| `01-sgemm-tutorial/src/kernels/tiled_sgemm.cuh` | shared memory 分块 | 阶段 8 |
| `01-sgemm-tutorial/src/kernels/bank_conflict_free_sgemm.cuh` | 消除 bank conflict | 阶段 3、8 |
...
| `01-sgemm-tutorial/tests/test_sgemm.cu` | 正确性测试写法参考 | 阶段 8 |
| `02-tensorcraft-core/` | header-only 算子库组织方式 | 阶段 9 |
| `03-hpc-advanced/src/02_reduction/` | reduction 实验 | 阶段 4 |
| `03-hpc-advanced/src/03_gemm/` | GEMM 实验 | 阶段 8 |
| `03-hpc-advanced/src/04_convolution/` | 卷积 | 阶段 6 之后 |
...
阶段 7~8 关注 tiled GEMM、合并访问、shared memory、warp shuffle、算术强度、roofline、寄存器和 occupancy。阶段 13 之后关注 FP8/FP4 量化与推理组件。性能结论必须配套重复计时和 profiler 指标。
