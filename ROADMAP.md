...
路线按“能运行 -> 能测量 -> 能解释 -> 能修改”推进。方括号是学习进度：`[ ]` 未开始，`[-]` 进行中，`[x]` 已完成。当前处于阶段 0：已完成 `task-001-vector-add`，进行中 `task-002-transpose-coalescing`。每次只推进一个 task，下一项任务由助教放在 `experiments/stage-x/task-y-*`。
...
- [-] 完成 vector add、transpose、reduction、scan、histogram 的正确性实验。（vector add 已完成；transpose 进行中：`stage-0/task-002-transpose-coalescing`）
