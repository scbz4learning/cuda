...
- 没有目标硬件时可以先完成代码阅读和正确性任务，并在汇报结论时说明限制；无需填写固定格式的结果文件。
...
阶段完成必须同时满足：正确性有可核验的输出证据，涉及性能结论时测量条件完整，学习者能用自己的话解释结果。证据可在对话或学习者自行选择的位置提供，不要求填写 `result.md`。助教只在这些证据出现后更新本文件；单纯读完链接不算完成。
